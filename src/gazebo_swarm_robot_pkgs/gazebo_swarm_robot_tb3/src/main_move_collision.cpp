...
int main(int argc, char** argv) {
    ...
    Eigen::VectorXd cos_a(swarm_robot_id.size());
    Eigen::VectorXd sin_a(swarm_robot_id.size());
    Eigen::VectorXd consist_item(swarm_robot_id.size());
    Eigen::VectorXd theta(swarm_robot_id.size());
    Eigen::VectorXd modifier(swarm_robot_id.size());
    ...
    while(! is_conv) {
        ...
        for (int i = 0; i < swarm_robot_id.size(); ++i) {
            ...
            for (int j = 0; j < swarm_robot_id.size(); ++j) {
                if (i == j) continue;
                dist = (current_robot_pose[i][0] - current_robot_pose[j][0]) * (current_robot_pose[i][0] - current_robot_pose[j][0])
                        + (current_robot_pose[i][1] - current_robot_pose[j][1]) * (current_robot_pose[i][1] - current_robot_pose[j][1]);
                if (dist < min_dist) min_dist = dist;
            }
            modifier(i) = std::max(0.0, 1.0 / min_dist - 0.3);
        }

        cos_a = (cur_pos.block(0, 2, del_pos.rows(), 1)).array().cos();
        sin_a = (cur_pos.block(0, 2, del_pos.rows(), 1)).array().sin();
        consist_item = cur_pos.block(0, 0, del_pos.rows(), 1).array() * cos_a.array() + cur_pos.block(0, 1, del_pos.rows(), 1).array() * sin_a.array();
        del_consist_item = -lap * consist_item;
        del_theta = del_pos.col(2) - Eigen::MatrixXd::Identity(swarm_robot_id.size(), swarm_robot_id.size()) * modifier;
