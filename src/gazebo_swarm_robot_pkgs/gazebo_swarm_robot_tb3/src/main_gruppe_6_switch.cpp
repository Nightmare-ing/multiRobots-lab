...
int main(int argc, char** argv) {
    ...
    double radius = 0.5;
    Eigen::MatrixXd targ_pos_normal(num_robots, 2);
    targ_pos_normal << radius*std::cos(30.0 / 180.0 * M_PI), radius* std::sin(30.0 / 180.0 * M_PI),
                       radius*std::cos(-30.0 / 180.0 * M_PI), radius* std::sin(-30.0 / 180.0 * M_PI),
                       radius*std::cos(90.0 / 180.0 * M_PI),  radius* std::sin(90.0 / 180.0 * M_PI),
                       radius*std::cos(-90.0 / 180.0 * M_PI), radius* std::sin(-90.0 / 180.0 * M_PI),
                       radius*std::cos(150.0 / 180.0 * M_PI), radius* std::sin(150.0 / 180.0 * M_PI),
                       radius*std::cos(-150.0 / 180.0 * M_PI), radius* std::sin(-150.0 / 180.0 * M_PI);

    Eigen::MatrixXd targ_pos_narrow(num_robots, 2);
    targ_pos_narrow << radius* std::cos(30.0 / 180.0 * M_PI), radius* std::sin(30.0 / 180.0 * M_PI),
                       radius* std::cos(-30.0 / 180.0 * M_PI), radius* std::sin(-30.0 / 180.0 * M_PI),
                       radius* std::cos(90.0 / 180.0 * M_PI) * 1/2, radius* std::sin(90.0 / 180.0 * M_PI) * 1/2,
                       radius* std::cos(-90.0 / 180.0 * M_PI) * 1/2, radius* std::sin(-90.0 / 180.0 * M_PI) * 1/2,
                       radius* std::cos(150.0 / 180.0 * M_PI), radius* std::sin(150.0 / 180.0 * M_PI),
                       radius* std::cos(-150.0 / 180.0 * M_PI), radius* std::sin(-150.0 / 180.0 * M_PI);
    /* should narrow ? */
    ...
    while(! is_conv) {
        ...
        if (!should_narrow) {
            del_pos = -lap * (cur_pos.leftCols(2) - targ_pos_normal);
        }
        else {
            del_pos = -lap * (cur_pos.leftCols(2) - targ_pos_narrow);
        }
